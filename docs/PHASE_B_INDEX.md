# PHASE_B_INDEX

_Last updated: 2026-01-08_

המסמך הזה מתאר את היישום הטכני (HOW) של NADU בהתאם ל־SPECS שהם החוק העליון. כל סטייה מה־SPECS מחייבת עדכון SPECS לפני שינוי יישומי.

## מסמכים כלולים
- 10_ARCHITECTURE.md
- 20_SECURITY.md
- 30_DATA_MODEL.md
- 40_API_CONTRACTS.md
- 50_CHAT_TECHNICAL.md
- 60_NOTIFICATIONS.md
- 70_SCALING_PERFORMANCE.md
- CI_CD.md
- 80_OBSERVABILITY_INCIDENTS.md
- 90_COST_OPTIMIZATION.md
- 100_ADMIN_MOD_TOOLS.md
- 110_TESTING_QA.md
- 120_BACKUP_DR.md
- 130_COMPLIANCE_TRUST.md
- 140_MEDIA_PIPELINE.md
- 150_SEARCH_FEED.md
- 160_MIGRATIONS_VERSIONING.md

## הערות
- אם משהו מתנגש עם SPECS — מעדכנים SPECS קודם.

